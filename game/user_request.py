...
class TowerDeveloper:
    ...
    def update(self, user_request: str, model):
        ...
        if user_request == "upgrade" and model.selected_tower.level < 5:
            ...
            if model.money > model.selected_tower.get_upgrade_cost():     #如果現有money大於升級所需
                model.money -= model.selected_tower.get_upgrade_cost()    #將現有money減掉升級所需
                model.selected_tower.level += 1                           #將塔升級
